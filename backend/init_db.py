...
def fill_db():
    ...
    def generate_products(n):
        ...
        for _ in range(n):
            products.append((
                ...
                round(random.uniform(10.0, 30000.0), 2),
    ...
    def generate_support_tickets(n, customer_ids, issue_descriptions):
        ...
        for _ in range(n):
            tickets.append((
                ...
                fake.date_this_year(),
                random.choice(["Open", "In Progress", "On Hold", "Resolved"]),
            ))
        return tickets
    ...
    support_tickets = generate_support_tickets(
        1000, list(range(1, 234)), issue_descriptions
    )

    # Insert data into the database
    cursor.executemany(
        "INSERT INTO Customers (CompanyName, ContactName, ContactEmail, Phone, Address, City, PostalCode, Country) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        customers,
    )
    cursor.executemany(
    ...
    cursor.executemany(
        "INSERT INTO OrderStatus (StatusName) VALUES (?)", order_statuses
    )
    cursor.executemany(
    ...
    cursor.executemany(
        "INSERT INTO SupportTicketStatus (StatusName) VALUES (?)",
        support_ticket_statuses,
    )
    cursor.executemany(
        "INSERT INTO SupportTickets (CustomerID, IssueDescription, DateCreated, Status) VALUES (?, ?, ?, ?)",
        support_tickets,
    )
